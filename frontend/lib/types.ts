...
export type DeliveryReportArtifact = { filename: "report.md"; format: "md"; contentType: "text/markdown; charset=utf-8"; sizeBytes: number; sha256: string };
export type DeliveryManifestItem = { kind: "final_model" | "report" | "metadata"; ready: boolean; required: boolean; filename?: string; format?: string; contentType?: string; generatedWithPackage?: boolean; sizeBytes?: number; downloadUrl?: string };
...
export type DeliveryPackage = { id: string; projectId: string; version: number; filename: string; sizeBytes: number; createdAt: string; downloadUrl: string; reportArtifact?: DeliveryReportArtifact };
export type DeliveryManifest = { ready: boolean; projectId: string; items: DeliveryManifestItem[]; missingRequired: string[]; notes: string[]; packageFilename: string; packageVersion: string; packageFormatVersion?: string; latestPackage?: DeliveryPackage | null; downloadable: boolean; downloadUrl: string | null; metadataPreview: DeliveryMetadataPreview; finalModelQuality?: DeliveryFinalModelQuality };
...
export type ModelPreviewDiagnostics = { available: boolean; enabled: boolean; blender_path: string | null; blender_exists: boolean; notes: string[] };
export type ModelPreviewStatus = { status: "not_started" | "running" | "completed" | "failed" | string; source_artifact_id?: string | null; viewer_artifact_id?: string | null; stage?: string | null; progress?: number | null; progress_percent?: number | null; started_at?: string | null; updated_at?: string | null; error_message?: string | null; job_progress?: JobProgress | null };
